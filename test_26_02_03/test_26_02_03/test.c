...
//	int a[5] = { 1, 2, 3, 4, 5 };
//	int* ptr = (int*)(&a + 1);
//	printf("%d, %d", *(a + 1), *(ptr - 1)); //打印结果为：2, 5
//
//	//&a是数组的地址，加1跳过整个数组，因此ptr指向a[5]（也就是最后一个元素的后面）
//	//a是数组名，是a[0]的地址，加1是a[1]的地址，解引用找到元素2
//	//ptr - 1是a[4]的地址，解引用找到元素5
//	return 0;
//}
...
//int main()
//{
//	p = (struct Test*)0x100000; //假设p的值是0x100000，那么下列表达式的值分别是多少
//	printf("%p\n", p + 0x1);
//	printf("%p\n", (unsigned long)p + 0x1);
//	printf("%p\n", (unsigned int*)p + 0x1);
//
//	//p是结构体类型，结构体占20个字节，加1是跳过一个结构体，因此加上20字节
//	//那么p + 0x1 = 0x100000 + 0x14 = 0x100014（20转成十六进制是14）
//
//	//将p强制类型转换成无符号长整型，相当于把p看出一个无符号长整型数字，加1就是加1个字节
//	//因此(unsigned long)p + 0x1 = 0x100000 + 0x1 = 0x100001
//
//	//将p强制类型转换成无符号整形指针类型，指针指向的类型是4个字节，加1就是加4个字节
//	//因此(unsigned int*)p + 0x1 = 0x100000 + 0x4 = 0x100004
//	return 0;
//}
...
//	
//	//在x86（32位平台），还可以勉强运行，结果为：4, 2000000
//	//在x64（64位平台），程序会崩溃，或者直接无法运行，因为int和int*大小不匹配
//
//	//&a + 1拿到a[4]的地址，存到ptr1中，减1再解引用拿到a[3]，也就是4
//	//(int)a是把数组首元素地址当成整数，加1就是整数加1，再看成地址存到ptr2中
//	//此时ptr2中存放了：00 00 00 02，其指向如下
//	// 
//	// 01 00 00 00 02 00 00 00 03 00 00 00 04 00 00 00
//	//    ↑
//	//   ptr2（向后访问4个字节是：00 00 00 02）
//	// 
//	//ptr2中保存的是指针，也就是地址02000000，用十六进制打印结果为2000000
//	return 0;
//}
